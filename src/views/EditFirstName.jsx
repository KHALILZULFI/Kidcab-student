...
const EditFirstName = (props) => {
    ...
    const check_valid = () => {
        if (first_name) {
            ...
            dropDownAlertRef.alertWithType('error', 'Validation error', 'Please enter your first name');
        }
    }
    ...
    const call_profile_update = () => {
        ...
        axios({
            ...
            .then(async response => {
                setLoading(false);
                dropDownAlertRef.alertWithType('success', 'Successfully updated', 'Your first name has been updated.');
                go_back();
            })
            .catch(error => {
                setLoading(false);
    ...
    return (
        <SafeAreaView style={{ backgroundColor: colors.lite_bg, flex: 1 }}>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center' }}>
                <Text numberOfLines={1} style={{ color: colors.theme_fg_two, fontSize: f_xl, fontFamily: bold }}>Enter your first name</Text>
                <View style={{ margin: 5 }} />
                <Text numberOfLines={1} style={{ color: colors.grey, fontSize: f_xs, fontFamily: normal }}>You need enter your first name</Text>
                <View style={{ margin: 20 }} />
                <View style={{ width: '80%' }}>
