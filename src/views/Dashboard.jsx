  ...
  const add_favourite_api = async () => {
    ...
    await axios({
      ...
      .then(async (response) => {
        setLoading(false);
        if (response.data.status == 1) {
          dropDownAlertRef.alertWithType('success', 'Success', 'Location added in your favourite pot!');
    ...
    await axios({
      ...
      .then(async (response) => {
        if (response.data.result == 0) {
          setLoading(false);
          dropDownAlertRef.alertWithType('error', 'Not Available', 'Our service is not available in your location.!');
        } else {
      ...
      .then(async (response) => {
        ...
        if (response.data.status == 1) {
          ...
          if (response.data.booking_type == 2) {
            dropDownAlertRef.alertWithType('success', 'Booking placed successfully', 'You can see you bookings in my rides menu.');
          }
          // booking_exit();
        } else {
          dropDownAlertRef.alertWithType('error', 'Sorry', 'Driver not available, try again');
        }
      })
      .catch((error) => {
        setLoading(false);
